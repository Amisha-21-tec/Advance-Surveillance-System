...
// Read the last detected location from the location.txt file
$lastDetectedLocation = "";
if (file_exists("location.txt")) {
    $lastDetectedLocation = file_get_contents("location.txt"); // Read the content of the file
}

// Function to get media from the directory
function getMedia($directory) {
    return glob("$directory/*.{jpg,jpeg,png,mp4,avi,mov}", GLOB_BRACE); // Capture images and video formats
}

// Directories where images and videos are saved
$accident_image_dir = 'C:/xampp/htdocs/Final Project Code/accident_images'; // Adjust the path as necessary
$accident_clip_dir = 'C:/xampp/htdocs/Final Project Code/accident_clips'; // Adjust the path as necessary

// Get media from both directories
$accident_media_images = getMedia($accident_image_dir);
$accident_media_videos = getMedia($accident_clip_dir);

// Merge all media into one array
$mediaFiles = array_merge($accident_media_images, $accident_media_videos);

// Count new media for notification
$mediaCount = count($mediaFiles);

// Initialize or retrieve the session variable for previous media count
if (!isset($_SESSION['previous_media_count'])) {
    $_SESSION['previous_media_count'] = 0; // Initialize on first load
}

$newMediaCount = $mediaCount - $_SESSION['previous_media_count'];

// Update the previous media count for the next page load
$_SESSION['previous_media_count'] = $mediaCount;

// Function to get media file creation time and convert to IST
function getMediaTime($mediaPath) {
    return date("Y-m-d H:i:s", filemtime($mediaPath)); // Get the file modification time in IST
}
?>
...
<body>
    ...
    <div class="logo">
        ...
        <div class="nav-links">
            ...
            <a class="active" href="#">Accident</a>
            ...
            <div class="notification-icon">
                ...
                <?php if ($newMediaCount > 0): ?>
                    <span class="notification-count"><?php echo $newMediaCount; ?></span>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <table class="detection-table" id="detectionTable">
            ...
            <tbody>
                <?php
                // Loop through the media and display them in the table
                foreach ($mediaFiles as $mediaFile) {
                    // Convert the absolute path to a relative path for the browser
                    $relativePath = str_replace('C:/xampp/htdocs', '', $mediaFile);
                    $relativePath = str_replace('\\', '/', $relativePath); // Converts backslashes to forward slashes

                    // Get the file extension to determine if it's an image or video
                    $extension = pathinfo($mediaFile, PATHINFO_EXTENSION);

                    // Get the time the media file was created in IST
                    $mediaTime = getMediaTime($mediaFile);

                    // Dynamically fill in the 'Location' and 'Alert was sent to' fields
                    echo "<tr>";

                    // If it's an image
                    if (in_array($extension, ['jpg', 'jpeg', 'png'])) {
                        echo "<td><img src='$relativePath' alt='Detection Image' width='150'></td>";
                    }
                    // If it's a video
                    elseif (in_array($extension, ['mp4', 'avi', 'mov','mp3'])) {
                        echo "<td>
                    ...
                    // Add other columns (location, email, time, and button) for both images and videos
                    echo "
                        <td>Pen</td>
                        <td>amishavartak1980@gmail.com</td>
                        <td>$mediaTime</td>
                        <td><button type='button' onclick='viewMedia(\"$relativePath\")'>View</button></td>
                    </tr>";
                }
                ?>
            </tbody>
    ...
    <script>
        function viewMedia(mediaPath) {
            // Open the media (image or video) in a new tab
        ...
        document.getElementById('searchBtn').addEventListener('click', function () {
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                const isDateMatch = rowDateOnly >= startDate.toISOString().split('T')[0] && rowDateOnly <= endDate.toISOString().split('T')[0];
